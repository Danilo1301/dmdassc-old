...
class TBBackpack {
  ...
  static GetItemInfo(url)
  {
    ...
    return new Promise(function(resolve) {
      TBRequest.GetBody(url).then((body)=>{
        ...
        info.sell_offers = self.GetOffers(sl_sell_orders);
        info.buy_offers = self.GetOffers(sl_buy_orders);
  ...
  static GetOffers(or) {
    ...
    while ((lastn = or.indexOf("<li class='listing ' id=", lastn)) != -1) {
      ...
      offer.paint = s.indexOf("data-paint_name") != -1 ? findAttribute(s, 'data-paint_name') : "";
      offer.datetime = new Date(findAttribute(s, 'datetime', true)).getTime();
      ...
      offer.user_name = findAttribute(user_s, 'data-name');
      offer.online = findAttribute(user_s, 'data-online') == "1";
...
function findAttribute(str, attr, onlyOneB) {
  var sm = onlyOneB ? "'" : '"';
  var find = `${attr}=${sm}`;
